...
public class PlayerServerCMD extends Command
{
    ...
    public PlayerServerCMD(PlayerServers pl, String s) {
        super(s, (String)null, new String[] { "pserver", "psrv", "ps" });
    ...
    public void startCooldown(String s) {
        ...
        this.proxy.getScheduler().schedule(this.pl, new Runnable() {
            @Override
            public void run() {
    ...
    private void addQueue(ProxiedPlayer proxiedPlayer, String s) {
        if (this.pl.useQueue) {
    ...
    private void sendHelp(CommandSender commandSender) {
        if (commandSender instanceof ProxiedPlayer) {
            ...
            if (hasPerm || this.pl.utils.hasPerm(string, "playerservers.ps.join")) {
                this.pl.utils.helpMessage((CommandSender)proxiedPlayer, "/ps join <player>", this.pl.msgMap.get("help-ps-join"));
            }
            this.pl.utils.helpMessage((CommandSender)proxiedPlayer, "/ps leave", this.pl.msgMap.get("help-ps-leave"));
            if (hasPerm || this.pl.utils.hasPerm(string, "playerservers.ps.create")) {
                this.pl.utils.helpMessage((CommandSender)proxiedPlayer, "/ps create [world]", this.pl.msgMap.get("help-ps-create"));
                this.pl.utils.helpMessage((CommandSender)proxiedPlayer, "/ps home", this.pl.msgMap.get("help-ps-home"));
                this.pl.utils.helpMessage((CommandSender)proxiedPlayer, "/ps stop", this.pl.msgMap.get("help-ps-stop"));
                this.pl.utils.helpMessage((CommandSender)proxiedPlayer, "/ps delete", this.pl.msgMap.get("help-ps-delete"));
                this.pl.utils.helpMessage((CommandSender)proxiedPlayer, "/ps worlds", this.pl.msgMap.get("help-ps-worlds"));
            }
            if (hasPerm || this.pl.utils.hasPerm(string, "playerservers.ps.motd")) {
                this.pl.utils.helpMessage((CommandSender)proxiedPlayer, "/ps motd [message]", this.pl.msgMap.get("help-ps-motd"));
            }
            if (this.pl.useExpiry && !this.pl.utils.hasPerm(string, "playerservers.bypassexpire") && (hasPerm || this.pl.utils.hasPerm(string, "playerservers.ps.checktime"))) {
                this.pl.utils.helpMessage((CommandSender)proxiedPlayer, "/ps time", this.pl.msgMap.get("help-ps-time"));
            }
            if (this.pl.useExpiry && !this.pl.utils.hasPerm(string, "playerservers.bypassexpire") && (hasPerm || this.pl.utils.hasPerm(string, "playerservers.ps.sharetime"))) {
                this.pl.utils.helpMessage((CommandSender)proxiedPlayer, "/ps sharetime", this.pl.msgMap.get("help-ps-sharetime"));
            }
        }
        else {
    ...
    public void execute(CommandSender commandSender, String[] array) {
        if (array.length < 1) {
            if (commandSender instanceof ProxiedPlayer && this.pl.usingHelper.containsKey(commandSender)) {
                this.pl.sender.controlGUI((ProxiedPlayer)commandSender);
            }
            else {
    ...
    public void leaveCommand(CommandSender commandSender, String[] array) {
        ProxiedPlayer proxiedPlayer = (ProxiedPlayer)commandSender;
        String string = proxiedPlayer.getUniqueId().toString();
    ...
    public void worldsCommand(CommandSender commandSender, String[] array) {
        ProxiedPlayer proxiedPlayer = (ProxiedPlayer)commandSender;
        String string = proxiedPlayer.getUniqueId().toString();
        ...
        if (this.pl.serverManager.hasServer(string)) {
            if (array.length < 2 || !array[1].equalsIgnoreCase("confirm")) {
                ...
                if (!this.pl.utils.isPortOpen(this.pl.utils.getSrvIp(string), this.pl.utils.getSrvPort(string))) {
                    ...
                    this.pl.serverManager.deleteServer((CommandSender)proxiedPlayer, string);
                }
                else {
                    this.pl.serverManager.deleteServer((CommandSender)proxiedPlayer, string);
                }
    ...
    public void motdCommand(CommandSender commandSender, String[] array) {
        String string = ((ProxiedPlayer)commandSender).getUniqueId().toString();
        if (array.length > 1) {
            ...
            if (this.pl.utils.stripColor(sb.toString()).length() > 20) {
    ...
    public void shareTimeCommand(ProxiedPlayer proxiedPlayer, String[] array) {
