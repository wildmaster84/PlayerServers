...
public class SettingsGUI extends CustomGUI
{
    ...
    public String doPlaceholders(String s) {
        try {
            if (s.contains("%expire-date%")) {
                s = s.replaceAll("%expire-date%", this.pl.expireDate);
            }
            if (s.contains("%time-left%")) {
                s = s.replaceAll("%time-left%", this.pl.utils.timeLeft());
            }
        }
    ...
    public ItemStack buildExpireTracker() {
        ...
        while (iterator.hasNext()) {
            lore.add(this.pl.utils.color(this.doPlaceholders(iterator.next())));
        }
    ...
    public void open(Player player, Inventory reopenGUI) {
        reopenGUI = this.reopenGUI(player, reopenGUI, 4, this.getTitle());
        if (reopenGUI == null) {
            return;
        }
        this.fill(reopenGUI);
